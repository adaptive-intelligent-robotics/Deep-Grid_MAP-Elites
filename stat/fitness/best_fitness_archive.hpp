namespace sferes {
  namespace stat {
    ...
    SFERES_STAT(Archive_best_save, Stat)
    {
    public:

      template <typename E> void refresh(const E& ea)
      {
       // Apply it only if the depth is greater than 1 as it concerns the best indiv
       if (Params::stochastic::deep_size > 1)
	...
	for(auto cell : filled_content)
	  {
		...
		if ((!_use_moved_fitness()
		    ...
		    && boost::fusion::at_c<PHANTOM_BEST_MODIFIER>(ea.fit_modifier()).is_moved_empty(ea, desc_center)) )
		  {
		    ...
		    // Add fitness to line
		    if (_use_moved_fitness())
		      write += std::to_string(boost::fusion::at_c<PHANTOM_BEST_MODIFIER>(ea.fit_modifier())
    ...
    SFERES_STAT(Archive_best_original_save, Archive_best_save)
    {
    public:
      Archive_best_original_save() { this->_name_prefixe = "Archive_Best_individual_Fitness-"; }
    protected:
    ...
    // Save archive moved
    SFERES_STAT(Archive_best_moved_save, Archive_best_save)
    {
    public:
      Archive_best_moved_save() { this->_name_prefixe = "Archive_Best_individual_Fitness_stable-"; }
    protected:
      bool _write_moved_cell() override { return true; };
    ...
    // Save archive empty
    SFERES_STAT(Archive_best_empty_save, Archive_best_save)
    {
    public:
      Archive_best_empty_save() { this->_name_prefixe = "Archive_Best_individual_Fitness_empty-"; }
    protected:
