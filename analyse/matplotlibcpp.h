...
PyObject* airl_colormap()
{
  ...
  for(size_t i=0; i<nb_col; i++){
    ...
    color.push_back({
		      (sin(x*2*M_PI*0.9 + 1.3*M_PI) / 2.0+0.5)*0.8+0.1,
		       sin(x*2*M_PI*0.9)/2*0.6+0.60,
		       1-(sin(x*2*M_PI*0.9+1.6*M_PI)/2+0.5)});
  }
	     ...
	     ) // The marker size in points**2
{
    ...
    PyDict_SetItemString(kwargs, "s", PyLong_FromLong(s));
    PyDict_SetItemString(kwargs, "c", zarray);
    PyDict_SetItemString(kwargs, "marker", PyString_FromString(m.c_str()));
...
template<typename NumericX, typename NumericY, typename NumericZ, typename NumericW>
bool scatter(const std::vector<NumericX>& x,
             const std::vector<NumericY>& y,
	     const std::vector<NumericZ>& z,
	     const std::vector<NumericW>& s,
	     bool show_colormap=false
	     ) // The marker size in points**2
{
    assert(x.size() == y.size());
    ...
    PyObject* yarray = get_array(y);
    PyObject* zarray = get_array(z);
    PyObject* warray = get_array(s);

    PyObject* kwargs = PyDict_New();
    PyDict_SetItemString(kwargs, "s", warray);
    PyDict_SetItemString(kwargs, "c", zarray);
...
template<typename NumericX, typename NumericY, typename NumericZ, typename NumericV>
bool scatter(const std::vector<NumericX>& x,
             const std::vector<NumericY>& y,
	     const std::vector<NumericZ>& z,
	     const std::vector<NumericV>& c,
	     const std::string m,
	     bool show_colormap=false
	     ) 
{
  ...
  if (!mpl_toolkitsmod) {
    ...
    PyObject* yarray = get_array(y);
    PyObject* zarray = get_array(z);
    PyObject* varray = get_array(c);

    PyObject* kwargs = PyDict_New();
    PyDict_SetItemString(kwargs, "c", varray);
    PyDict_SetItemString(kwargs, "marker", PyString_FromString(m.c_str()));
