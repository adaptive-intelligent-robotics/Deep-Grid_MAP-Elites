#ifndef NAIVE_SAMPLING_GRID
#define NAIVE_SAMPLING_GRID

namespace sferes {
  namespace qd {
    namespace container {

      SFERES_STOCHASTIC_GRID(Naive_sampling_grid)
      {
      public:
	typedef boost::shared_ptr<Phen> indiv_t;
	typedef boost::shared_ptr<Cell> cell_t;
	
	const size_t n = Params::stochastic::N_sampling-1;	
	...
	bool add(indiv_t indiv)
	{
	  ...
	  // n evaluations
	  this->_total_eval += n;
	  indiv->fit().eval(*indiv, n);

	  // Check again if indiv need to be added
	  if (this->_not_valid(indiv))
	    return false;
